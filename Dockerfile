...
RUN apt-get update \
    ...
    && cd /usr/local && aria2c https://github.com/BtbN/FFmpeg-Builds/releases/download/latest/ffmpeg-master-latest-linux64-gpl.tar.xz && tar --strip-components=1 -xvaf ffmpeg-master-latest-linux64-gpl.tar.xz -C /usr/local/ && rm -rf /usr/local/ffmpeg-master-latest-linux64-gpl.tar.xz /usr/local/doc /usr/local/LICENSE.txt \
